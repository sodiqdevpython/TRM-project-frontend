...
function AddOrganizationScreen() {
    ...
    const [regions, setRegions] = useState([{ id: -1, name: "Barchasi" }]);
    const [profiles, setProfiles] = useState([]);  // New state for profiles

    async function getProfiles() {
        const response = await axios.get(`${baseURL}/profile/?is_active=true&is_selected=false&is_super_admin=false`);
    ...
    async function postData(event) {
        ...
        if (!orgLocName) {
            alert("Shahar yoki tuman nomini tanlang.");
            return;
        }
        ...
        if (isCity && orgLocName) {
            form.append('city', orgLocName);
        } else if (!isCity && orgLocName) {
        ...
        <div className='flex flex-col'>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4 font-rubik'>
                ...
                <div className='flex flex-col py-2'>
                    ...
                    <select onChange={(e) => selectedLocation(e.target.value)} name='select' className='p-2 border shadow hover:shadow-2xl transition-all duration-300'>
                        <option value="-1">Tanlang</option>
                ...
                <div className='flex flex-col py-2'>
                    ...
                    {loadingProfile ? (
                        ...
                        <Select
                            ...
                            onChange={(option) => setAdmin(option?.value ?? null)}
                        />
                    )}
                </div>
            </div>
            ...
            {/* Yuborish */}
            <div className='py-4'>
                <DefaultButton onClick={postData} label='Yuborish' />
            </div>
        </div>
    );
}
